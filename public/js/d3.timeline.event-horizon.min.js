function timeline(a,c){var e=d3.min(c,function(k){return k.start}),b=d3.max(c,function(k){return k.end}),f=e-1,d=b+1;this.ctx=c;var h=[],g=[];for(var j in this.ctx){var i=this.ctx[j].start;i&&$.inArray(i,h)==-1&&h.push(i)}for(j in this.ctx)(i=this.ctx[j].end)&&$.inArray(i,g)==-1&&$.inArray(i,h)==-1&&g.push(i);this.beginnings=h;this.endings=g;this.ctx.unshift({name:"before and after",start:f,end:d});this.container=a;this.min=e;this.max=b;this.padding=20;this.before=f;this.after=d;this.h_years=this.h_labels=
...
b.width)}).attr("transform",function(b){if(b.name!="before and after"){var f=a.measure_text(b);f=Math.floor(a.h_years-f.width);return"rotate("+[0,c(b),e-f-13].join(",")+")"}}).style("text-anchor","center").attr("id",function(b){return"date-"+b}).attr("class",function(){return"timeline-event timeline-event-year"})};
...
function(b){if(b.name!="before and after"){var f=a.measure_text(b);f=Math.floor(a.h_years-f.width);return"rotate("+[0,c(b),e-f-13].join(",")+")"}}).style("text-anchor","center").attr("id",function(b){return"date-"+b}).attr("class","timeline-event timeline-event-year")};
...
function(d){if(d.name!="before and after")return"rotate("+[-45,c(d.start),e-(b+f+10)].join(",")+")"}).attr("class","timeline-event timeline-event-item")};
